    ...
    for(RangeIterator r=mesh->iterateBndryLowerY(); !r.isDone(); r++) {
      for(int jz=0; jz<mesh->ngz; jz++) {
        for(int jy = mesh->ystart-1;jy >= 0; jy--) {
          ...
          //phi(r.ind, jy, jz) = phi(r.ind, jy+1, jz) + 2.*( Pe(r.ind, jy, jz) - Pe(r.ind, jy+1, jz) ) / (Ne(r.ind, jy+1, jz) + Ne(r.ind, jy, jz) );
          
          /*
          phi(r.ind, jy, jz) = phi(r.ind, jy+1, jz) + phi(r.ind, jy+2, jz) - phi(r.ind, jy+3, jz);
          Ne(r.ind, jy, jz) = Ne(r.ind, jy+1, jz) + Ne(r.ind, jy+2, jz) - Ne(r.ind, jy+3, jz);
          Pe(r.ind, jy, jz) = Pe(r.ind, jy+1, jz) + Pe(r.ind, jy+2, jz) - Pe(r.ind, jy+3, jz);
          */
...
//           NVi(r.ind,mesh->yend+1,jz) = Ne(r.ind,mesh->yend+1,jz) * Vi(r.ind,mesh->yend+1,jz);
  ...
  if(currents) {
    ...
    if(boussinesq) {
      ...
      ddt(Vort) -= Div_n_bxGrad_f_B_XPPM(Vort, phi, vort_bndry_flux, poloidal_flows);
    }else {
      ...
      ddt(Vort) -= 0.5*Div_n_bxGrad_f_B_XPPM(Vort + tmp, phi, vort_bndry_flux, poloidal_flows);
  ...
  if(currents && (electromagnetic || FiniteElMass)) {
    ...
    // Advection
    //ddt(VePsi) -= Div_par_FV_FS(NelimVe*Ve, Ve, sqrt(mi_me)*sound_speed) / NelimVe;
  ...
  if(sheath_ydown) {
	...
	for(int jz=0; jz<mesh->ngz; jz++) {
	  ...
	  // Heat flux
	  BoutReal q = (sheath_gamma - 3) * tesheath * nesheath * Cs; // NB: positive
  ...
  if(neutral_model > 0) {
    ...
    case 1: {
      ...
      for(int i=0;i<mesh->ngx;i++)
        for(int j=0;j<mesh->ngy;j++)
          for(int k=0;k<mesh->ngz;k++) {
            ...
            BoutReal sigma_cx = Nelim(i,j,k)*Nnorm*hydrogen.chargeExchange(Telim(i,j,k)*Tnorm)/Omega_ci;
	  
            // Ionisation frequency, normalised to ion cyclotron frequency
            BoutReal sigma_iz = Nelim(i,j,k)*Nnorm*Nn(i,j,k) * hydrogen.ionisation(Telim(i,j,k)*Tnorm)/Omega_ci;
            ...
            // Rates
            BoutReal R_rc = SQ(Nelim(i,j,k)) * hydrogen.recombination(Nelim(i,j,k)*Nnorm, Telim(i,j,k)*Tnorm) * Nnorm / Omega_ci; // Time normalisation
            BoutReal R_iz = Nelim(i,j,k)*Nn(i,j,k) * hydrogen.ionisation(Telim(i,j,k)*Tnorm) * Nnorm / Omega_ci; // Time normalisation
            ...
            Q(i,j,k) += (3./2)*(Telim(i,j,k)*R_rc - Tn(i,j,k)*R_iz);
      ...
      if(neutral_friction) {
        // Vorticity
        if(boussinesq) {
          ddt(Vort) -= Div_Perp_Lap_FV( F/(Nelim * SQ(mesh->Bxy)), phi, vort_bndry_flux);
        }else {
	 ...
	 Nn = max(Nelim) * exp(-ll);
      ...
      for(int i=mesh->xstart;i<=mesh->xend;i++) {
	    ...
	    vth_n = sqrt(Tn(i,j,k)) * Cs0; // in m/s
     	    sigma_cx = Nelim(i,j,k) * Nnorm * hydrogen.chargeExchange(Telim(i,j,k)*Tnorm); // in seconds
      	    sigma_iz = Nelim(i,j,k) * Nnorm * Nn(i,j,k) * hydrogen.ionisation(Telim(i,j,k)*Tnorm);  // in seconds
	  ...
	  fluxout = 0.5 * (NVi(i,mesh->yend,k) + NVi(i,mesh->yend+1,k)); // [d^-2][t^-1]
	  ...
	  nnexp = 0.0;
          for(int j=mesh->ystart;j<=mesh->yend;j++) {
	    sigma_iz = hydrogen.ionisation(Telim(i,j,k)*Tnorm) * Nnorm / Omega_ci; // ionization rate [d^3]/[t]
      ...
      for(int i=mesh->xstart;i<=mesh->xend;i++)
        for(int j=mesh->ystart;j<=mesh->yend;j++)
          for(int k=0;k<mesh->ngz;k++) {
            ...
            BoutReal R_rc = SQ(Nelim(i,j,k)) * hydrogen.recombination(Nelim(i,j,k)*Nnorm, Telim(i,j,k)*Tnorm) * Nnorm / Omega_ci; // Time normalisation
            BoutReal R_iz = Nelim(i,j,k)*Nn(i,j,k) * hydrogen.ionisation(Telim(i,j,k)*Tnorm) * Nnorm / Omega_ci; // Time normalisation
            BoutReal R_cx = Nelim(i,j,k)*Nn(i,j,k) * hydrogen.chargeExchange(Telim(i,j,k)*Tnorm) * Nnorm / Omega_ci;
    ...
    case 3: {
      ...
      for(RangeIterator r=mesh->iterateBndryLowerY(); !r.isDone(); r++) {
        ...
        for(int jz=0;jz<mesh->ngz-1;jz++) {
          flux_ion -= frecycle * 0.5*(Ne(r.ind,mesh->ystart,jz) + Ne(r.ind,mesh->ystart-1,jz)) * 0.5*(Ve(r.ind,mesh->ystart,jz) + Ve(r.ind,mesh->ystart-1,jz)); // Flux through surface [m^-2 s^-1], should be positive since Ve < 0.0
        }
        flux_ion /= mesh->ngz-1; // Average over Z
        
        // Flow of neutrals inwards
        BoutReal flow = flux_ion * (mesh->J(r.ind, mesh->ystart) + mesh->J(r.ind, mesh->ystart-1)) / (sqrt(mesh->g_22(r.ind,mesh->ystart))+ sqrt(mesh->g_22(r.ind,mesh->ystart-1)));
        ...
        ddt(Pn2D)(r.ind,mesh->ystart) += dndt * (3.5/Tnorm); // Franck-Condon energy
      ...
      for(RangeIterator r=mesh->iterateBndryUpperY(); !r.isDone(); r++) {
        ...
        BoutReal flux_ion = 0.0;
        for(int jz=0;jz<mesh->ngz-1;jz++) {
          flux_ion += frecycle * 0.5*(Ne(r.ind,mesh->yend,jz) + Ne(r.ind,mesh->yend+1,jz)) * 0.5*(Ve(r.ind,mesh->yend,jz) + Ve(r.ind,mesh->yend+1,jz)); // Flux through surface [m^-2 s^-1], should be positive
        }
        flux_ion /= mesh->ngz-1; // Average over Z
        ...
        ddt(Pn2D)(r.ind,mesh->yend) += dndt * (3.5/Tnorm); // Franck-Condon energy
      ...
      if(neutral_friction) {
        // Vorticity
        if(boussinesq) {
          ddt(Vort) -= Div_Perp_Lap_FV( Fvort/(Nelim * SQ(mesh->Bxy)), phi, vort_bndry_flux);
        }else {
          ddt(Vort) -= Div_Perp_Lap_FV( Fvort/SQ(mesh->Bxy), phi, vort_bndry_flux);
        }
      }
    ...
    case 4: {
      ...
      if(neutral_friction) {
        // Vorticity
        if(boussinesq) {
          ddt(Vort) -= Div_Perp_Lap_FV( (Rcx + Rrc)/(Nelim * SQ(mesh->Bxy)), phi, vort_bndry_flux);
        }else {
          ddt(Vort) -= Div_Perp_Lap_FV( (Rcx + Rrc)/SQ(mesh->Bxy), phi, vort_bndry_flux);
        }
      }
      ...
      if(sheath_ydown) {
        for(RangeIterator r=mesh->iterateBndryLowerY(); !r.isDone(); r++) {
          ...
          for(int jz=0;jz<mesh->ngz-1;jz++) {
            ...
            ddt(Pn)(r.ind,mesh->ystart,jz) += dndt * (3.5/Tnorm); // Franck-Condon energy
            ddt(NVn)(r.ind,mesh->ystart,jz) += dndt * neutral_vwall * sqrt(3.5/Tnorm);
      ...
      if(sheath_yup) {
        for(RangeIterator r=mesh->iterateBndryUpperY(); !r.isDone(); r++) {
          ...
          for(int jz=0;jz<mesh->ngz-1;jz++) {
            ...
            ddt(Pn)(r.ind,mesh->yend,jz) += dndt * (3.5/Tnorm); // Franck-Condon energy
            ddt(NVn)(r.ind,mesh->yend,jz) -= dndt * neutral_vwall * sqrt(3.5/Tnorm);
...
const Field3D Hermes::H(const Field3D &f, BoutReal h) { // Diffusion operator
  ...
  return 
    - D4DY4_FV(h*SQ(SQ(mesh->dy)), f) 
    //- h*SQ(SQ(mesh->dx))*D4DX4(f)
    //- h*SQ(SQ(mesh->dz))*D4DZ4(f)
    ;
}

const Field3D Hermes::H(const Field3D &f, BoutReal h, const Field3D &mask) { // Diffusion operator
  if(h < 0.0)
    return 0.0;
  
  return
    - D4DY4_FV(h*mask*SQ(SQ(mesh->dy)), f) 
